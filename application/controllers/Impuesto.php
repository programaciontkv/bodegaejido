	...
	public function nuevo(){
		if($this->permisos->rop_insertar){
			...
			$data=array(
						...
						'action'=>base_url().'impuesto/guardar'
						);
	...
	public function guardar(){
		...
		if($this->form_validation->run()){
			...
			if($this->impuesto_model->insert($data)){
				...
				redirect(base_url().'impuesto');
			}else{
				$this->session->set_flashdata('error','No se pudo guardar');
				redirect(base_url().'impuesto/nuevo');
			}
		}else{
			$this->nuevo();
		}	
	...
	public function editar($id){
		if($this->permisos->rop_actualizar){
			$data=array(
						...
						'action'=>base_url().'impuesto/actualizar'
						);
			$this->load->view('layout/header',$this->menus());
			$this->load->view('layout/menu',$this->menus());
			$this->load->view('impuesto/form',$data);
	...
	public function actualizar(){
		...
		if($this->form_validation->run()){
			...
			if($this->impuesto_model->update($id,$data)){
				...
				redirect(base_url().'impuesto/editar'.$id);
			}
		}else{
			$this->editar($id);
		}	
